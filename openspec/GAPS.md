...
**🔴 S3. Admin-креденшел: token чи mTLS-cert — дві живі історії.**
R3.8: «адмін автентифікується mTLS client-cert, НЕ token/PoP». Водночас task-8 (§794),
G5-мітигація і R3.7 описують видачу **one-time admin-токена**. Не зведено: що саме лежить
у `0600`-файлі, що robить bootstrap, який шлях перевіряє DoD G5. Ризик: імплементують обидва
шляхи → подвоєна поверхня привілейованого доступу без lifecycle для одного з них.
→ Рішення потрібне ДО `add-invites-admin` (task 2.1 там).
...
**🟡 S12. План застарів відносно коду (доc-drift, не безпека).**
...
| Severity | IDs | Статус |
|---|---|---|
| 🔴 | S3, S8 | потребують рішення/задачі ДО старту відповідних changes |
| 🟠 | S1, S2, S4, S5, S6, S7, S9, S10 | закриваються канонізацією в changes (зроблено) + task-рядками |
| 🟡 | S11, S12 | зафіксовані; S11 — тримати на оці при receive/events |
