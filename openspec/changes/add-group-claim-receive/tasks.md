...
## 2. Claim-флоу
- [ ] 2.1 `requestGroupClaim()` (T3: без аргументу-групи): one-time код, TTL ~10 хв, bound лише (U), rate-limit + per-identity quota (вкл. cap активних bindings)
- [ ] 2.2 Verify (T2/T3): код у TTL → атомарний consume-and-bind (W20-патерн); binding = (U, G-де-з'явився-код, anchorAci вставника)
...
- [ ] 2.5 Membership churn (T2): перед send через бот перевірити anchorAci ∈ members(G) (кеш ~60с + інвалідизація по group-update подіях); не член → відмова + інвалідизація binding'у; binding має TTL як backstop
- [ ] 2.6 Самокерування bindings (T3): `listMyBindings` (лише власні, без чужих даних) + `revokeMyBinding` — юзер прибирає binding, створений вставленням його коду в небажану групу
...
- [ ] 4.6 Membership churn (S10/T2, anchor = U): U вставив власний код у G, потім вигнаний із G → наступний send відхиляється, binding інвалідований
...
- [ ] 4.8 Anchor-churn (T2, proxy-випадок): код U вставив P; P вийшов із G → send U відхиляється, binding інвалідований (право живе з поручителем)
- [ ] 4.9 T3: consume-and-bind фіксує групу за місцем появи коду; той самий код у другій групі після consume/TTL → відмова; `revokeMyBinding` прибирає небажаний binding
