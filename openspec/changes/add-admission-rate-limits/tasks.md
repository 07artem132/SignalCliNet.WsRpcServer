...
## 5. Тести (DoD)
- [x] 5.1 G3: N паралельних send під бюджетом=1 → рівно 1 проходить
- [x] 5.2 G2: один юзер не голодоморить решту (floor спрацьовує)
- [x] 5.3 Бюджет переживає рестарт; краш форфейтить ≤K, ніколи не over-send (W13)
- [x] 5.4 G6: флуд повільних send → cap тримає, pending не росте unbounded
- [x] 5.5 Msg >64KB → `MessageTooBig`; auth-timeout закриває неавтентиф. сокет
- [x] 5.6 Перевищення квоти → `-32005 data.retry_after` (in-band) / WS-close `4429`; браузерний клієнт бачить retry_after (S9)
